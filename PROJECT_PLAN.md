...
| Run manifests and static reports | Local command lineage implemented; production coverage open | Formal local commands emit immutable JSON/Markdown artifacts, run manifests, checkpoints, and latest pointers; golden summary/report artifacts are registered; real full-season/report collection and prospective operations remain unproven |
...
| Paper betting ledger | Contract/storage/derived registry implemented; prospective operation open | Real/open market gates, exposure recomputation, append-only revisions, deterministic settlement, immutable summary artifacts, and recompute run manifests are covered; no real prospective ledger cohort is recorded |
